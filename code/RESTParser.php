...
abstract class RESTParser extends Object
{
    ...
    public static function matches_signature($body)
    { /* NOP */
    }

    public function parseInto($request, $fields, $noun = null, $defaultType = null)
    {
        ...
        if ($noun) {
            if ($type && !($noun instanceof $type)) {
                throw new Exception('Mismatched API types while parsing request', 400);
            }
        } else {
            if (!$type) {
                throw new Exception('No API type specified', 400);
            }
            if (!class_exists($type)) {
                throw new Exception('Invalid API type "'.$type.'"', 400);
            }
            $noun = new $type();
        }
        if (in_array('*', $fields)) {
...
{
    public static $mimetypes = array('application/x-www-form-urlencoded', 'multipart/form-data');
